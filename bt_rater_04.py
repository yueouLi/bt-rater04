...
df = pd.read_csv("bt_batch_04.csv", encoding="utf-8-sig")
df.columns = [col.strip() for col in df.columns]  # 去除列名首尾空格

st.set_page_config(page_title="Backtranslation Rating - Rater 04", layout="centered")
st.title("📝 Simplification Back-Translation Evaluation - Rater 04")
...
if st.button("💾 Export Ratings to CSV"):
    pd.DataFrame(ratings).to_csv(f"bt_ratings_rater04.csv", index=False, encoding="utf-8-sig")
